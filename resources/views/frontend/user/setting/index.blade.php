                ...
                <div class="tab-area mb-3">
                    <nav class="nav nav-tabs bg-sky-2 rounded flex-column flex-sm-row flex-wrap text-center text-sm-start">
                        <a href="{{ route('user.settings.profile') }}"
                            class="nav-link flex-sm-fill text-dark {{ isActive('user.settings.profile') }}"
                            aria-controls="profile" aria-selected="true"> <i class="fas fa-user"></i> {{ __('Profile') }} </a>
                        <a href="{{ route('user.settings.2fa.setup') }}"
                            class="nav-link flex-sm-fill text-dark {{ isActive('user.settings.2fa.setup') }}"
                            aria-controls="security" aria-selected="false"> <i class="fas fa-shield-alt"></i>
                            {{ __('2FA Security') }} </a> <a href="{{ route('user.settings.password.change') }}"
                            class="nav-link flex-sm-fill text-dark {{ isActive('user.settings.password.change') }}"
