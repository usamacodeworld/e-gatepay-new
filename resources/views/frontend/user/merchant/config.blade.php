...
@section('merchant_collapse_active', 'active_li_class')
@section('merchant_collapse_show', 'show')
@section('content')
    <main class="admin-main">

        <div class="single-form-card">
            <div class="card-title d-flex flex-column flex-md-row justify-content-between">
                <h6 class="text-white mb-2 mb-md-0"> <i class="fas fa-cogs me-2"></i> {{ __('Merchant API Configuration') }}
                </h6>
            ...
            <div class="card-main">
                ...
                {{-- Security Warning --}}
                <div class="text-muted bg-light-warning fw-500 border-warning-left-5 rounded text-white p-3 mb-4 d-flex align-items-center"
                ...
                {{-- API Benefits --}}
                <div
                    class="d-flex flex-column flex-md-row align-items-start align-items-md-center border-bottom pb-3 mb-3 w-100 justify-content-between">
                    <div class="element-text-box">
                        <h6 class="main-title mb-1">
                            <span
                                class="me-2">🚀</span>{{ __('Benefits of :name API Integration', ['name' => setting('site_title')]) }}
                        </h6>
                        <p class="mb-0">
                ...
                <div id="credentials-section">
                    @foreach (\App\Enums\EnvironmentMode::cases() as $env)
                        <div class="credentials-container" id="{{ $env->value }}-credentials"
                            ...
                            @php
                                $credentials = [
                                    'merchant_key' => [
                                        ...
                                        'value' => $env->isSandbox()
                                            ? $merchant->test_merchant_key
                                            : $merchant->merchant_key,
                                        'copy_title' => $env->isSandbox()
                                            ? __('Copy Test Merchant Key')
                                            : __('Copy Merchant Key'),
                                    ],
                                    'api_key' => [
                            ...
                            @foreach ($credentials as $credential)
                                <div
                                    class="d-flex flex-column flex-md-row align-items-start align-items-md-center border-bottom mb-4 w-100 justify-content-between">
                                    <div class="element-text-box pe-0 pe-md-5 mb-2 mb-md-0 w-100">
                                        ...
                                        <p
                                            class="mb-0 text-muted text-break d-flex flex-column flex-md-row align-items-start align-items-md-center">
                ...
                {{-- API Documentation --}}
                <div
                    class="d-flex flex-column flex-md-row align-items-start align-items-md-center pt-3 mt-3 w-100 justify-content-between">
                    ...
                    <a href="{{ route('api-docs.index') }}" target="_blank" class="btn btn-light-primary btn-sm">
...
<script>
    $('.environment-btn').on('click', function() {
        ...
        // Show loading state
        $(this).html('<i class="fas fa-spinner fa-spin me-1"></i> {{ __('Switching...') }}').prop(
            'disabled', true);
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    notifyEvs('error', response.message ||
                        '{{ __('Failed to switch environment') }}');
                }
            },
            error: function() {
                notifyEvs('error',
                    '{{ __('An error occurred while switching environment') }}');
            },
            complete: function() {
